...
{
    public class GFaceRecognizer : IFaceRecognizer
    {
        bool inited = false;
        ...
        public float Compare(byte[] pFea1, byte[] pFea2)
        {
            ...
            try
            {
                ...
                result = GFaceSdk.GFace6_Compare(nThreadID, pFea1, pFea2);
        ...
        public bool Init()
        {
            try
            {
                if (!inited)
                {
                    //added by frank 2017-09-13
                    //string path = AppDomain.CurrentDomain.BaseDirectory + "\\seeta_fd_frontal_v1.0.bin";
                    //inited = GFaceSdk.init(path);


                    inited = GFaceSdk.GFace6_Init("");
        ...
        public IntPtr GetVersion()
        {
            try
            {
                IntPtr version = GFaceSdk.GFace6_GetVer();
                return version;
            }
            catch
            { return (IntPtr)0; }
        ...
        public bool CheckKey()
        {
            try
            {
                return GFaceSdk.GFace6_CheckKey();
            }
            catch(Exception e)
            {
                return false;
        ...
        private bool GFace_GetFea(byte[] picFiles, int nW, int nH, int nThreadID, out byte[] pFea)
        {
            ...
            //lock (objFea)
            {
                ...
                {
                    LogHelper.WriteLog("GFace6_GetFea（GetFea）获取特征码结束，发生异常....." + ex.ToString());
                    return false;
                }
                ...
                try
                {
                    ...
                    {
                        LogHelper.WriteLog("GFace6_GetFea（GetFea）申请到的内存为空指针.....");
                    }
                    else
                    {
                        lock (objFea)
                        {
                            flag = GFaceSdk.GFace6_GetFea(0, pRgb24, nW, nH, 1, faceTempPoint, ptrFea);
        ...
        public bool Face_GetFeaByFile(byte[] files, int nW, int nH, out byte[] pPea)
        {
            ...
            try
            {
                ...
                //lock (objlock)
                {
                    if (GFace6_GetFeaCerti(pRgb24, pRgb8, nW, nH, 0, out pPea))
                    {
        ...
        private bool GFace6_GetFeaCerti(byte[] pRgb24, byte[] pRgb8, int nW, int nH, int nThreadID, out byte[] pFea)
        {
            //LogHelper.WriteLog("（GetFea）获取特征码开始.....");
            pFea = new byte[1024];
            FaceRectInfo[] FaceRect = new FaceRectInfo[100];
        ...
        private int GFaceDetectCerti(byte[] pGrayBuf, int nWidth, int nHeight, out FaceRectInfo[] rect, out FacePointInfo[] facePoint)
        {
            //lock (objFea)
            {
                ...
                try
                {
                    ...
                    lock (objFea)
                    {
                        nFaceNum = GFaceSdk.GFace6_Detect(0, pGrayBuf, nWidth, nHeight, ptrRect, ptrFaceRect);
                    }
        ...
        public int Face_DetectByFile(byte[] files, int nWidth, int nHeight, out FaceRectInfo[] rect, out FacePointInfo[] facePoint)
        {
            //lock (objFea)
            {
                ...
                try
                {
                    ...
                    lock (objFea)
                    {
                        nFaceNum = GFaceSdk.GFace6_Detect(0, pRgb8, nWidth, nHeight, ptrRect, ptrFaceRect);
                    }
                    rect = new FaceRectInfo[nFaceNum];
                    facePoint = new FacePointInfo[nFaceNum];
                    for (int i = 0; i < nFaceNum; i++)
